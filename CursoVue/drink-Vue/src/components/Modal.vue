...
//* Queremos renderizar la lista de ingredientes de cada  receta ... pero en el endpoint muestra que cada receta tiene diferente cantindad de ingredientes
                        ...
                        <DialogPanel
                            ...
                            <div>
                                <div class="mt-3">
                                    ...
                                    <DialogTitle as="h3" class="text-gray-900 text-4xl">
                                        {{ bebidas.receta.strDrink }}
                                    </DialogTitle>
                                    <img :src="bebidas.receta.strDrinkThumb" :alt="` Imagen de ${bebidas.receta.strDrink}`"
                                        class="mx-auto w-96  m-5" />
                                    <DialogTitle as="h3" class="text-gray-900 text-4xl m-2">
                                        Ingredientes y Cantidades
                                    </DialogTitle>
                                    <!-- * Solo utilizarlo cuando sepamos que los datos son seguros -->
                                    <div v-html="formatearIngredientes().outerHTML"></div>
                                    <p class="text-lg text-gray-600">{{ bebidas.receta.strInstructions }}</p>
                            ...
                            <div class="mt-5 sm:mt-6 flex justify-between gap-4">
                                <button type="button"
                                    class="w-full rounded bg-gray-600 hover:bg-gray-500 p-3 font-bold uppercase text-white shadow "
                                    @click="modal.handleClickModal()">Cerrar</button>
                                <button type="button"
                                    class="w-full rounded bg-orange-600 hover:bg-orange-500 p-3 font-bold uppercase text-white shadow "
                                    @click="favoritos.handleClickFavorito()">{{ modal.textBoton }}</button>
                            </div>
                        </DialogPanel>
                    </TransitionChild>
                </div>
